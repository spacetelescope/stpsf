...
class NIRSpec_three_MSA_shutters(poppy.AnalyticOpticalElement):
    ...
    def get_transmission(self, wave):
        ...
        w_inside_2 = np.where(
            (abs(y) > (msa_height / 2) + msa_wall) & (abs(y) < msa_height * 1.5 + msa_wall) & (abs(x) < (msa_width / 2))
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        poppy.CompoundAnalyticOptic.__init__(
            ...
            (
                ...
                poppy.CircularAperture(radius=39 * pupil_mag / 2, *args, **kwargs),
            ),
            name='CLEARP',
        )
...
class NIRCam_BandLimitedCoron(poppy.BandLimitedCoron):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.offset_lwb = {
            filt: self.get_bar_offset_from_siaf(filt, channel='LW')
            for filt in [
                'F250M',
                'F300M',
                'F277W',
    ...
    def get_bar_offset_from_siaf(self, filt, channel='LW'):
        ...
        offset_arcsec = np.sqrt(
            (self.siaf.apertures[refapername].V2Ref - self.siaf.apertures[apername].V2Ref) ** 2
            + (self.siaf.apertures[refapername].V3Ref - self.siaf.apertures[apername].V3Ref) ** 2
        )
...
def _width_blc(desired_width, approx=None, plot=False):
    ...
    if approx is None:
        ...
        sigma = np.linspace(approx * 0.9, approx * 1.1, 100000.0)
...
def _calc_blc_wedge(deg=4, wavelength=2.1e-6):
    ...
    difflim = wavelength / 6.5 * 180.0 * 60 * 60 / np.pi
...
def _trim_nan_image(xgrid, ygrid, zgrid):
    ...
    while np.isnan(zgrid2.sum()):
        ...
        xind_good = np.where((np.abs(col_diff) <= ndiff) & (num_nans_cols < 0.5 * nrows))[0]
        yind_good = np.where((np.abs(row_diff) <= ndiff) & (num_nans_rows < 0.5 * ncols))[0]
        # get border limits
        x1, x2 = (xind_good.min(), xind_good.max() + 1)
        y1, y2 = (yind_good.min(), yind_good.max() + 1)
...
def _fix_zgrid_NaNs(xgrid, ygrid, zgrid, rot_ang=0):
    ...
    func = RegularGridInterpolator((ygrid2, xgrid2), zgrid2, method='linear', bounds_error=False, fill_value=None)
...
class WebbFieldDependentAberration(poppy.OpticalElement):
    ...
    def __init__(self, instrument, include_oversize=False, **kwargs):
        ...
        self.name = "{instrument} internal WFE at V2V3=({v2:.2f},{v3:.2f})', near {field_point}".format(
            instrument=lookup_name, field_point=self.row['field_point_name'], v2=telcoords_am[0], v3=telcoords_am[1]
        )
        self.si_wfe_type = ('Interpolated', 'SI WFE was interpolated between available meas.')
        ...
        for i in range(1, 37):
            ...
            if np.isnan(cf):
                if i == 1:
                    self.si_wfe_type = ('Extrapolated', 'SI WFE was extrapolated outside available meas.')
                ...
                func = RegularGridInterpolator((ygrid, xgrid), zgrid, method='linear', bounds_error=False, fill_value=None)
        ...
        if include_oversize:
            ...
            self.opd = poppy.zernike.opd_from_zernikes(coeffs, npix=npix, aperture=self.amplitude, outside=0)
        else:
...
class NIRCamFieldAndWavelengthDependentAberration(WebbFieldDependentAberration):
    ...
    def __init__(self, instrument, **kwargs):
        ...
        sw_focus_cf = (
            np.array([-5.169185169, 50.62919436, -201.5444129, 415.9031962, -465.9818413, 265.843112, -59.64330811])
            / defocus_to_rmswfe
        )
...
class LookupTableFieldDependentAberration(poppy.OpticalElement):
    ...
    def __init__(
        self,
        instrument,
        field_points_file=None,
        phasemap_file=None,
        which_exercise='MIMF_KDP_2',
        add_niriss_defocus=None,
        rm_ptt=None,
        rm_center_ptt=None,
        add_mimf_defocus=False,
        add_sm_defocus=False,
        nwaves=None,
        **kwargs,
    ):
        ...
        self.name = '{instrument} at V2V3=({v2:.2f},{v3:.2f}) Lookup table WFE from ({v2t:.2f},{v3t:.2f})'.format(
            instrument=self.instr_name, v2=telcoords_am[0], v3=telcoords_am[1], v2t=self.row['V2'], v3t=self.row['V3']
        )
        self.si_wfe_type = ('Lookup Table', 'SI + OTE WFE from supplied lookup table of phase maps.')
        ...
        if rm_ptt:
            ...
            if rm_center_ptt:
                ...
                coeffs_per_si = {
                    'NIRCam A': [-3.50046880e-10, -7.29120639e-08, -1.39751567e-08],
...
class NIRCamFieldDependentWeakLens(poppy.OpticalElement):
    ...
    def __init__(
        ...
        name='WLP8',
        instrument=None,
        center_fp_only=False,
        verbose=False,
        include_power=True,
        include_astigmatism=True,
        **kwargs,
    ):
        ...
        self.opd = poppy.zernike.opd_from_zernikes(zernike_coefficients, npix=npix, outside=0)
        self.amplitude = np.ones_like(self.opd)
