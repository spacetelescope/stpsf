...
tm_parameters = (
    [1 * u.day, 'SM', None, START_ANGLE, END_ANGLE, GLOBAL_FOCUS],
    ...
    [5 * u.hour, 'A4', None, 5.0, 15.0, COEFFS_A4],
)


@pytest.mark.parametrize('time, seg, scaling, start_angle, end_angle, coeff_truth', tm_parameters)
def test_get_thermal_slew_coeffs(time, seg, scaling, start_angle, end_angle, coeff_truth):
...
def test_thermal_slew_partial_angle():
    ...
    assert np.allclose(
        1e9 * cf_full, 1e9 * cf_tot
    ), 'should get same total coefficients for one big slew or if broken into two parts'
...
def test_thermal_slew_update_opd():
    ...
    assert np.isclose(
        np.max(otelm.opd) / 1e-9, expected_max, rtol=1e-3
    ), 'OPD max does not match expected value after 1 day slew.'
...
def test_apply_field_dependence_model():
    ...
    ote._apply_field_dependence_model(
        assume_si_focus=False
    )  # Do this test directly on the OTE OPD, without any implicit focus adjustments
...
def test_get_zernike_coeffs_from_smif():
    ...
    assert np.allclose(otelm._get_hexike_coeffs_from_smif(0.0, 0.0), np.asarray([0.0] * 9))
    ...
    telfer_zern = [
        ...
        -0.0036747300,
        0.0033910640,
    ]  # Taken from Telfer's tool
    # Convert Telfer's Zernikes to Hexikes:
    hexikes = [
        -telfer_zern[1],
        2.0 * telfer_zern[0] - (60984.0 / 69531.0) * telfer_zern[5],
        telfer_zern[2],
        (33.0 / 25) * telfer_zern[3],
        (-33.0 / 25) * telfer_zern[4],
        (1386.0 / 860.0) * telfer_zern[5],
    ]
    ...
    telfer_zern_mirim_fp1mimf = np.asarray(
        [-0.25066019, 0.22840080, -0.53545999, -0.024227464, -0.0025191352, 0.00050082553]
    )  # Taken from Telfer's tool
    # Convert Telfer's Zernikes to Hexikes:
    hexikes = hexikes = [
        -telfer_zern_mirim_fp1mimf[1],
        2.0 * telfer_zern_mirim_fp1mimf[0] - (60984.0 / 69531.0) * telfer_zern_mirim_fp1mimf[5],
        telfer_zern_mirim_fp1mimf[2],
        (33.0 / 25) * telfer_zern_mirim_fp1mimf[3],
        (-33.0 / 25) * telfer_zern_mirim_fp1mimf[4],
        (1386.0 / 860.0) * telfer_zern_mirim_fp1mimf[5],
    ]

    otelm.segment_state[-1, :] = [300.0, 400.0, 100.0, 200.0, 5.0, 0.0]
...
def test_segment_tilt_signs(fov_pix=50, plot=False, npix=1024):
    ...
    psf_kwargs = {'monochromatic': 2e-6, 'fov_pixels': fov_pix, 'oversample': 1, 'add_distortion': False}
