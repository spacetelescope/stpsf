...
def load_wfe_budget_info():
    ...
    if wfe_budget_info is None:
        ...
        wfe_budget_filename = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'otelm', 'jwst_wfe_summary_from_optical_budget.csv'
        )
...
def visualize_wfe_budget(inst, slew_delta_time=14 * u.day, slew_case='EOL', ptt_only=False, verbose=True):
    ...
    wfe_for_imagemotion = poppy.zernike.opd_from_zernikes(
        [0, 0, 0, los_as_wfe.to_value(u.meter)], aperture=aperture, npix=1024, outside=0
    )
    ...
    show_opd(
        wfe_total,
        aperture,
        title=f'Total WFE for OTE+{loc}',
        ax=axes[0, 0],
        annotate_budget='System Performance',
        instname=inst.name,
        titlebold=True,
    )
    ...
    show_opd(
        wfe_si,
        aperture,
        title=f'{inst.name} internal WFE at {inst.detector}, {inst.detector_position}',
        ax=axes[3, 1],
        annotate_budget='SI internal WFE',
        instname=inst.name,
    )
    show_opd(
        wfe_si * 0,
    ...
    for irow in [1, 2, 3]:
        ...
        fig.text(
            between_col0_col1, meany_row, '=', color='purple', horizontalalignment='center', fontweight='bold', fontsize=18
        )
        fig.text(
            between_col1_col2, meany_row, '+', color='purple', horizontalalignment='center', fontweight='bold', fontsize=18
        )
        fig.text(
    ...
    for irow, symbol in enumerate(['=', '+', '+']):
        ...
        fig.text(
            meanx_col0, between_rows, symbol, color='royalblue', horizontalalignment='center', fontweight='bold', fontsize=18
        )
    ...
    fig.text(
        ...
        meany_row0 + 0.05,
        f'Optical model for {inst.name} wavefront error',
        fontweight='bold',
        fontsize=15,
    )
